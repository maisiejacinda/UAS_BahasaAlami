...
if model_absa is None or vectorizer_absa is None:
    ...
    uploaded_file = st.file_uploader(
        ...
        type=["csv"],
        key="csv_uploader_unique"  # Menambahkan key unik agar state upload CSV lebih stabil
    )
        ...
        try:
            ...
            if "ulasan" not in df.columns:
                ...
                for teks in df["ulasan"]:
                    ...
                    hasil_sentimen_csv.append(prediksi)

                    if len(aspek) == 0:
                        hasil_aspek_csv.append("Umum")
                    else:
                        hasil_aspek_csv.append(", ".join(sorted(set(aspek))))

                df["Sentimen"] = hasil_sentimen_csv
                df["Aspek"] = hasil_aspek_csv
                ...
                with col2:
                    ...
                    ax.axis("equal")
                    st.pyplot(fig)
                    plt.close(fig)  # Menutup figure matplotlib agar hemat memori server

                # Statistik Ringkasan
                st.subheader("📈 Ringkasan Data")
                c1, c2, c3 = st.columns(3)
                c1.metric("Jumlah Data", len(df))
                c2.metric("Jumlah Sentimen", len(sentiment_counts))
                c3.metric("Aspek Unik", len(set(df["Aspek"])))

                # Pemrosesan Bar Chart Aspek Terbanyak
                semua_aspek = []
                ...
                if len(semua_aspek) > 0:
                    ...
                    aspek_df = pd.DataFrame({
                        "Aspek": list(counter.keys()),
                        "Jumlah": list(counter.values())
                    })

                    st.subheader("📍 Aspek yang Paling Banyak Muncul")
                    st.bar_chart(aspek_df.set_index("Aspek"))
                    st.dataframe(aspek_df, use_container_width=True)
                else:
